...
# Decorators
from superclub.apps.posts.decorators import post_list_decorator
...
# Class Section
class ClubPostsViewSet(mixins.ListModelMixin,
                       GenericViewSet):
    serializers = {
    ...
    def get_queryset(self):
        if getattr(self, 'swagger_fake_view', False):
            return None
        queryset = Post.objects.filter(board__board_group__club=self.kwargs["club_pk"])
        return queryset

    @swagger_auto_schema(**post_list_decorator(title=_('4. 클럽 - Guest'), serializer=PostListSerializer))
